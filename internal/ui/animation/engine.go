...
// SetOnExerciseChange sets a callback fired when active exercise changes
...
// start cancels the previous run and launches a new animation goroutine
...
// runExercise selects and runs the requested exercise scenario
...
// notifyExerciseChange reports the current exercise to the overlay
...
// runDirectional alternates the sprite through center and target positions
...
// runMove performs one target-hold-return movement cycle
...
// runBlinkExercise alternates open and closed eye sprites until the deadline
...
// sleepWithContext waits for duration or stops early when the context is done
